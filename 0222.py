...
#
# 不同變數指派相同的 primitive type
#
myvar1 = 'y'
print('id(myvar1) =', id(myvar1)) 
# 新物件建立，其值是'y'，一個新的 reference/pointer 被建立指向這個新物件
# id(myvar1) = 1957762579568

myvar3 = 'y'
print(id(myvar3)) 
# 因為已有一物件有值'y'，所以傳入之前建立物件的 reference/pointer
# id 同 myvar1


#
# 相同變數指派不同的 primitive type
#
myvar = 1
# Assign 值在 Python 中就是 binding，我們將值 1 指派給 myvar
print('id(myvar) = ', id(myvar)) # id(myvar) =  1708050612464 
...
print('id(myvar) = ', id(myvar)) # id 變成 1972050137648 
...
myvar2 = myvar1
print(id(myvar2)) 
# 沒有建立新的物件，id 同 myvar1

myvar = myvar1
print(id(myvar)) 
# 沒有建立新的物件，id 同 myvar1

myvar1 = 'z' # 變更 myvar1 的值
print(id(myvar2)) # id 指向 myvar1 變更前的 id
print(id(myvar))  # 同上
...
class Student:
    ...
    def change_FAMILY(self, change):
        self.FAMILY['parent'] = change        

    def change_NAME(self, change):
        self.NAME = change  
        

Kim = Student('Kim', 39, 'female', 'KimFather')
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
# Kim name =  age = 0 sex =  family= {'parent': ''}

print('id(Kim.NAME) =', id(Kim.NAME))
print('id(Student.NAME) =', id(Student.NAME))
# 此時 id(Kim.NAME) = id(Student.NAME)

Leo = Student('Leo', 40, 'male', 'Leo Sr')
# Leo name = Leo age = 40 sex =  family= {'parent': 'Leo Sr'}
# 此時 id(Kim.NAME) = id(Student.NAME) != id(Leo.NAME) 因為 Leo.NAME 指派新的值了

print('Leo name =', Leo.NAME, 'age =', Leo.AGE, 'sex =', Leo.GENDER, 'family=', Leo.FAMILY)
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
# Leo name = Leo age = 40 sex =  family= {'parent': 'Leo Sr'}
# Kim name =  age = 0 sex =  family= {'parent': 'Leo Sr'}
...
print('Leo name =', Leo.NAME, 'age =', Leo.AGE, 'sex =', Leo.GENDER, 'family=', Leo.FAMILY)
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
# Leo name = Leo age = 40 sex =  family= {'parent': 'Tom'}
# Kim name =  age = 0 sex =  family= {'parent': 'Tom'}
# 同樣變更 Kim 的 FAMILY 也會變更到 Leo 的 FAMILY

Student.FAMILY['zip'] = '75252'
print('Leo name =', Leo.NAME, 'age =', Leo.AGE, 'sex =', Leo.GENDER, 'family=', Leo.FAMILY)
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
# Leo name = Leo age = 40 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# Kim name =  age = 0 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# 同樣如果變更 Class 的 FAMILY 也會變更到物件的 FAMILY

Student.AGE = 99
print('Leo name =', Leo.NAME, 'age =', Leo.AGE, 'sex =', Leo.GENDER, 'family=', Leo.FAMILY)
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
# Leo name = Leo age = 40 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# Kim name =  age = 99 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# 注意變更 Class 的 AGE 時，因為 Kim 尚未指派 AGE 所以此時 id(Kim.AGE) = id(Student.AGE)
# 所以變更 Student.AGE 同樣會影響 Kim 的 AGE，但不影響 Leo 的 AGE

Kim.change_NAME('Lisa')
print('id(Kim.NAME) =', id(Kim.NAME))  
print('Kim name =', Kim.NAME, 'age =', Kim.AGE, 'sex =', Kim.GENDER, 'family=', Kim.FAMILY)
print('Leo name =', Leo.NAME, 'age =', Leo.AGE, 'sex =', Leo.GENDER, 'family=', Leo.FAMILY)
# Kim name = Lisa age = 99 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# Leo name = Leo age = 40 sex =  family= {'parent': 'Tom', 'zip': '75252'}
# 所以變更 Student.AGE 同樣會影響 Kim 的 AGE，但不影響 Leo 的 AGE

# Python 物件會自動給予一個 __dict__ 屬性，裡面包含了一個屬性的 list


print(Student.__dict__)
# {'__module__': '__main__', 
#    'NAME': '', 'FAMILY': {'parent': 'Tom', 'zip': '75252'}, 
#    'AGE': 99, 
#    'GENDER': '', 
#    '__init__': <function Student.__init__ at 0x0000018B7EBB5D80>, 
#    'change_FAMILY': <function Student.change_FAMILY at 0x0000018B7EBB5E10>, 
#    'change_NAME': <function Student.change_NAME at 0x0000018B7EBB5EA0>, 
...
# 重點!!!
# 為什麼會有上述的情況呢? 因為在 Python 中的 dot 語法會自動呼叫 __getattribute__()，此方法會優先尋找物件的 internal dictionary(dict)，再來是物件的 type， 以上述的情況而言，如果要尋找 Kim.AGE，就是先執行 Kim.__dict__['AGE']，再來才是Kim.__class__.__dict__['AGE']
try:
    print(Kim.__dict__['AGE']) # 出現 KeyError: 'AGE，合理因為 Kim 的 dict 只有 NAME
    print(Kim.__class__.__dict__['AGE']) # 99
except KeyError:
    pass
